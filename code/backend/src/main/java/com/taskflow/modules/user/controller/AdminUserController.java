...
public class AdminUserController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getAdminStats() {
        List<UserEntity> allUsers = userRepository.findAll().stream()
                .filter(u -> !Boolean.TRUE.equals(u.getIsDeleted()) && !"DELETED".equalsIgnoreCase(u.getStatus()))
                .collect(Collectors.toList());

        long totalUsers = allUsers.size();
        long activeUsers = allUsers.stream()
                .filter(u -> !"LOCKED".equalsIgnoreCase(u.getStatus()) && !"BLOCKED".equalsIgnoreCase(u.getStatus()) && !"INACTIVE".equalsIgnoreCase(u.getStatus()))
                .count();
    ...
    public ResponseEntity<ApiResponse<UserDto>> resetUserPassword(
        ...
        if (newPassword == null || newPassword.trim().length() < 6) {
            throw new AppException(ResultCode.VALIDATION_ERROR, "Mật khẩu mới phải có ít nhất 6 ký tự");
        }

        user.setPassword(passwordEncoder.encode(newPassword.trim()));
        UserEntity saved = userRepository.save(user);
        return ResponseEntity.ok(ApiResponse.success("User password updated successfully", userMapper.toDto(saved)));
    }

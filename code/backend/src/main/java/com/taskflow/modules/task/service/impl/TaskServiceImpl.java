...
public class TaskServiceImpl implements TaskService {
    ...
    public List<TaskDto> getWorkspaceTasks(UUID userId, UUID workspaceId, String status, String priority, UUID assigneeId, String search, Boolean archived) {
        List<ProjectDto> projects = projectService.getWorkspaceProjects(userId, workspaceId, false, null);
        if (projects == null || projects.isEmpty()) {
            return new ArrayList<>();
        }
        List<TaskDto> allTasks = new ArrayList<>();
        for (ProjectDto project : projects) {
            allTasks.addAll(getProjectTasks(userId, project.getId(), status, priority, assigneeId, search, archived));
        }
        return allTasks;
    }

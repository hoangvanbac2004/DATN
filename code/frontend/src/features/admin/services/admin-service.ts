...
export const adminService = {
  ...
  updateUserRole: async (userId: string, roleName: 'ADMIN' | 'MANAGER' | 'USER'): Promise<AdminUserDto> => {
  ...
  changeUserPassword: async (userId: string, newPassword: string): Promise<void> => {
    await apiClient.put(`/admin/users/${userId}/password?newPassword=${encodeURIComponent(newPassword)}`);
  },

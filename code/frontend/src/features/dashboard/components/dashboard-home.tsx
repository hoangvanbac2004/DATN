...
export function DashboardHome() {
  ...
  // Derived metrics per role (All strictly scoped to activeWorkspace!)
  ...
  const endOfNextWeek = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 7, 23, 59, 59, 999);

  // Overdue tasks strictly in this workspace
  const workspaceOverdueTasks = workspaceTasks.filter((t) => {
    if (!t.dueDate) return false;
    const isDone = t.status === 'COMPLETED' || t.status === 'DONE';
    return new Date(t.dueDate) < now && !isDone;
  });

  // Today tasks strictly in this workspace
  const workspaceTodayTasks = workspaceTasks.filter((t) => {
    if (!t.dueDate) return false;
    const due = new Date(t.dueDate);
    return due >= startOfToday && due <= endOfToday;
  });

  // Upcoming tasks in this workspace
  const workspaceUpcomingTasks = workspaceTasks.filter((t) => {
    if (!t.dueDate) return false;
    const due = new Date(t.dueDate);
    const isDone = t.status === 'COMPLETED' || t.status === 'DONE';
    return due > endOfToday && due <= endOfNextWeek && !isDone;
  });
  ...
  const myOverdueTasks = workspaceOverdueTasks.filter(
    (t) => t.assignee?.id === user?.id || t.assignee?.email === user?.email
  );
  const myTodayTasks = workspaceTodayTasks.filter(
    (t) => t.assignee?.id === user?.id || t.assignee?.email === user?.email
  );
  ...
  // Workspace-scoped productivity stats (Current Week: Monday to Sunday)
  const workspaceProductivity = React.useMemo(() => {
    const toLocalDateStr = (dateInput?: string | Date | null) => {
      if (!dateInput) return '';
    ...
    // Filter tasks strictly for Staff vs Admin/Manager
    const relevantTasks = isStaffUser
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const completedCount = relevantTasks.filter((t) => {
        const isDone = t.status === 'COMPLETED' || t.status === 'DONE';
        const date = toLocalDateStr(t.updatedAt || t.createdAt);
        return isDone && date === dateStr;
      }).length;

      const createdCount = relevantTasks.filter((t) => {
        const date = toLocalDateStr(t.createdAt);
        return date === dateStr;
      }).length;
    ...
    return stats;
  }, [workspaceTasks, isStaffUser, user]);

  // Workspace-scoped rich recent activities
  const workspaceActivities = React.useMemo(() => {
    ...
    const relevantActivityTasks = isStaffUser
      ? workspaceTasks.filter(
          (t) =>
            t.assignee?.id === user?.id ||
            t.assignee?.email === user?.email ||
    ...
    return [...relevantActivityTasks]
      .sort((a, b) => new Date(b.updatedAt || b.createdAt || 0).getTime() - new Date(a.updatedAt || a.createdAt || 0).getTime())
      .slice(0, 8)
      .map((t) => {
        const isCompleted = t.status === 'COMPLETED' || t.status === 'DONE';
        const actionPrefix = getStatusLabel(t.status);
        const assigneeName = t.assignee?.fullName || t.assignee?.email || 'Chưa gán';
        const priorityText = getPriorityLabel(t.priority);
        const dueDateText = t.dueDate ? ` • Hạn: ${new Date(t.dueDate).toLocaleDateString('vi-VN')}` : '';

        return {
          id: t.id,
          type: isCompleted ? 'TASK_COMPLETED' : t.status === 'IN_PROGRESS' ? 'TASK_IN_PROGRESS' : 'TASK_CREATED',
          title: `${actionPrefix}: "${t.title}"`,
          description: `Giao cho: ${assigneeName} • Ưu tiên: ${priorityText}${dueDateText}`,
    ...
    <div className="space-y-6 text-text-primary">
        ...
        {isAdminUser && (
          <>
            ...
            <StatCard
              ...
              value={activeWorkspace ? (activeWorkspace.memberCount || 1) : 0}
            ...
            <StatCard
              ...
              value={workspaceOverdueTasks.length}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <ProductivityChart stats={workspaceProductivity} isStaff={isStaffUser} />
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-text-primary font-heading">
                  {isAdminUser
                    ? 'Công việc cần ưu tiên trong Workspace'
                    : isManagerUser
                ...
                <p className="text-[11px] text-text-secondary">
                  {isAdminUser
                    ? 'Danh sách công việc đang được thực hiện ở Workspace hiện tại'
                    : isManagerUser
            ...
            {/* Overdue Tasks Alert */}
            {(isStaffUser ? myOverdueTasks : workspaceOverdueTasks).length > 0 && (
              <div className="rounded-2xl border border-status-error/30 bg-status-error/5 p-4 space-y-3 shadow-xs">
                <div className="flex items-center justify-between border-b border-status-error/20 pb-2">
                  <h3 className="text-xs font-extrabold text-status-error flex items-center">
                    <AlertTriangle className="mr-1.5 h-4 w-4" /> Công việc Quá hạn (
                    {(isStaffUser ? myOverdueTasks : workspaceOverdueTasks).length})
                  </h3>
                  <span className="text-[10px] text-status-error font-bold">Cần xử lý gấp</span>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  {(isStaffUser ? myOverdueTasks : workspaceOverdueTasks).slice(0, 4).map((task) => (
            ...
            <div className="rounded-2xl border border-surface-border bg-surface p-5 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-surface-border pb-3">
                <h3 className="text-xs font-bold text-text-primary font-heading flex items-center space-x-2">
                  ...
                  <span>
                    Hạn chót hôm nay ({(isStaffUser ? myTodayTasks : workspaceTodayTasks).length})
                  </span>
                </h3>
              </div>

              {(isStaffUser ? myTodayTasks : workspaceTodayTasks).length === 0 ? (
                <div className="flex h-36 flex-col items-center justify-center rounded-xl border border-dashed border-surface-border bg-surface-alt/40 p-6 text-center">
                  ...
                  <p className="text-[11px] text-text-secondary mt-1">Tuyệt vời! Bạn đang duy trì tiến độ rất tốt.</p>
                </div>
              ) : (
                <div className="grid gap-3 sm:grid-cols-2">
                  {(isStaffUser ? myTodayTasks : workspaceTodayTasks).map((task) => (
                    <TaskCard key={task.id} task={task} onSelect={(t) => setSelectedTask(t)} />
                  ))}
                </div>
              )}
        ...
        <div className="space-y-6">
          ...
          <RecentActivity activities={workspaceActivities} />
        </div>
      </div>

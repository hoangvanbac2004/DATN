...
export function TaskDetailModal({ task, isOpen, onClose, onEdit }: TaskDetailModalProps) {
  ...
  const isMyOwnTask =
    !isStaff || // admin/manager can always edit
    task?.assignee?.id === currentUser?.id ||
    task?.assignee?.email === currentUser?.email;

  // Combined: can edit = isManagerOrAdmin, OR (isStaff AND is my task)
  const canEdit = isManagerOrAdmin || isMyOwnTask;
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-2xl border border-surface-border bg-surface-alt/30 p-5 space-y-3.5 shadow-xs">
                ...
                <div className="space-y-2 pt-1">
                  {checklists.map((item) => (
                    <div
                      ...
                      className={`flex items-center space-x-3 rounded-xl border border-surface-border bg-surface p-2.5 text-xs transition ${
                        canEdit
                    ...
                    >
                      <input
                        ...
                        className="h-4 w-4 rounded-xs border-surface-border text-primary focus:ring-primary cursor-pointer disabled:cursor-not-allowed"
                      />
                      <span
                ...
                {/* Add New Checklist Item Form - only for authorized users */}
                {canEdit ? (
                  ...
                  <div className="flex items-center space-x-2 pt-2 rounded-xl border border-dashed border-surface-border/50 px-3 py-2">
                    <Lock className="h-3 w-3 text-text-muted" />
                    <span className="text-[11px] text-text-muted italic">Bạn không có quyền chỉnh sửa checklist của công việc này.</span>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-6 border-t lg:border-t-0 lg:border-l border-surface-border lg:pl-6 pt-5 lg:pt-0">
              ...
              {/* Status Switcher - only editable by authorized users */}
              <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  {!canEdit && (
                    <span className="flex items-center space-x-1 text-[10px] font-semibold text-red-400 bg-red-500/10 px-2 py-0.5 rounded-md">
                      <Lock className="h-2.5 w-2.5" />
                  ...
                  {(['TODO', 'IN_PROGRESS', 'IN_REVIEW', 'DONE'] as TaskStatus[]).map((s) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        disabled={!canEdit}
                        onClick={() => canEdit && handleRequestStatusChange(s)}
                        title={!canEdit ? 'Bạn không có quyền thay đổi trạng thái công việc này' : undefined}
                        className={`rounded-xl py-2 px-2.5 text-xs font-bold border text-center transition duration-150 active:scale-95 ${
                          isCurrent
                            ? 'border-primary bg-primary text-white shadow-xs'
                            : !canEdit
                            ? 'border-surface-border bg-surface-alt/40 text-text-muted cursor-not-allowed opacity-50'
